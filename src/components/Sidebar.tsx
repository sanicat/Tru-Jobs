...
function SidebarItem({ to, icon, children }: { to: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <NavLink
      to={to}
      className={({ isActive }) =>
        `flex items-center gap-2 rounded-lg px-3 py-2 text-[16px] font-medium transition-colors ${
          isActive ? 'bg-blue-50 text-blue-700' : 'text-gray-700 hover:bg-gray-50'
        }`
      }
    >
      <span className={`shrink-0 ${/* icon color follows active state via text-current */ ''}`}>{icon}</span>
      <span>{children}</span>
    </NavLink>
  )
}
