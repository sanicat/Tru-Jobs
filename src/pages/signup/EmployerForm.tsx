...
const EmployerForm = () => {
  ...
  const [typeOfRecruiter, setTypeOfRecruiter] = useState<string | null>(null);
  ...
  const ready =
    company.trim() && typeOfRecruiter && position && fullName.trim() && email.trim() && password && confirm &&
    password === confirm && accepted;
    ...
    <div className="flex min-h-screen bg-white flex-col md:flex-row">
      ...
      <div className="relative md:hidden w-full h-56">
        ...
        <div className="absolute bottom-4 left-4 right-4 text-white z-10">
          <h2 className="text-xl font-bold mb-1">Build Your Dream Team</h2>
          <p className="text-white/90 text-sm">Hire smarter by discovering verified professionals and tracking performance.</p>
        </div>
      </div>
        ...
        <div className="flex-1 max-w-md mx-auto w-full flex flex-col justify-center">
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between relative">
              <h1 className="text-2xl font-bold text-gray-900 mt-0">Build Your Hiring Profile</h1>
              {/* Role Dropdown: Employer selected */}
              <div className="relative">
                ...
                {roleOpen && (
                  <div role="menu" className="absolute right-0 mt-2 w-44 rounded-md border bg-white shadow-md z-20 py-1">
                    <button
                      role="menuitem"
                    ...
                    <button
                      ...
                      onMouseDown={(e) => e.preventDefault()}
                    >
                      <Check className="w-4 h-4 text-blue-600" /> Employer
                    </button>
                  </div>
                )}
              </div>
            ...
            <p className="text-sm text-gray-600 mt-2">
              ...
              <Link to="/" className="text-blue-600 hover:text-blue-700 font-medium">Log In</Link>
            </p>
          </div>
          ...
          <form className="space-y-5">
            {/* Company Name */}
            <div>
              <Input id="company" type="text" placeholder="Company Name" value={company} onChange={(e) => setCompany(e.target.value)} />
            </div>

            {/* Type of Recruiter - Dropdown */}
            <div className="relative">
              <button
                type="button"
                className="w-full text-left px-3 py-2 text-sm rounded-md border bg-white hover:bg-gray-50 text-gray-700 inline-flex items-center justify-between"
                aria-haspopup="listbox"
                aria-expanded={recruiterOpen}
                onClick={() => setRecruiterOpen((o) => !o)}
              >
                <span>{typeOfRecruiter ?? 'Type of Recruiter'}</span>
                ...
                <div className="absolute z-20 mt-1 w-full bg-white border rounded-md shadow-xs">
                  {['Corporate Recruiter', 'Agency Recruiter', 'Hiring Manager'].map((opt) => (
                    <button
                      ...
                      onMouseDown={(e) => e.preventDefault()}
                      onClick={() => { setTypeOfRecruiter(opt); setRecruiterOpen(false); }}
                    >
                      {opt}
                    </button>
                  ))}
                ...
                <div className="absolute z-20 mt-1 w-full bg-white border rounded-md shadow-xs">
                  {['HR', 'Talent Acquisition', 'Operations', 'Founder'].map((opt) => (
                    <button
                      ...
                      className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50"
                      onMouseDown={(e) => e.preventDefault()}
                      onClick={() => { setPosition(opt); setPositionOpen(false); }}
                    >
                      {opt}
                    </button>
                  ))}
            ...
            {/* Confirm Password */}
            <div className="relative">
              <Input id="confirmPassword" type={showConfirm ? 'text' : 'password'} placeholder="Confirm Password" value={confirm} onChange={(e) => setConfirm(e.target.value)} />
              <button type="button" onClick={() => setShowConfirm((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600" aria-label="Toggle confirm password visibility">
            ...
            <label className="flex items-start gap-3 text-sm text-gray-600 leading-6">
              <Checkbox id="terms" checked={accepted} onCheckedChange={(v) => setAccepted(Boolean(v))} className="mt-0.5" />
              <span>
                I agree to the <a className="text-blue-600 hover:text-blue-700 font-medium" href="#">Terms and Conditions</a>, and{' '}
            ...
            <Button
              type="submit"
              className={`w-full py-3 text-sm ${ready ? 'bg-blue-600 hover:bg-blue-700 text-white cursor-pointer' : 'bg-gray-200 text-gray-500 hover:bg-gray-200 cursor-not-allowed'}`}
              disabled={!ready}
            >
              Register
            </Button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <SocialButton
                icon={<svg viewBox="0 0 24 24" className="w-5 h-5"><path fill="#0077B5" d="M19 3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14m-.5 15.5v-5.3a3.26 3.26 0 0 0-3.26-3.26c-.85 0-1.84.52-2.32 1.3v-1.11h-2.79v8.37h2.79v-4.93c0-.77.62-1.4 1.39-1.4a1.4 1.4 0 0 1 1.4 1.4v4.93h2.79M6.88 8.56a1.68 1.68 0 0 0 1.68-1.68c0-.93-.75-1.69-1.68-1.69a1.69 1.69 0 0 0-1.69 1.69c0 .93.76 1.68 1.69 1.68m1.39 9.94v-8.37H5.5v8.37h2.77z" /></svg>}
                provider="linkedin"
              >
                Sign up with LinkedIn
              </SocialButton>
              <SocialButton
                icon={<svg viewBox="0 0 24 24" className="w-5 h-5">
